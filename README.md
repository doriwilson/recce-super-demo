...
# Switch to PR and build automatically
./scripts/switch-pr.sh 1  # PR #1: Incremental model changes
./scripts/switch-pr.sh 2  # PR #2: Breaking change detection
./scripts/switch-pr.sh 3  # PR #3: Timestamp validation
...
- Discards uncommitted changes (clean demo state)
- Switches to PR branch
...
# Discard any uncommitted changes (demo mode)
git reset --hard HEAD
...
**Note**: This is a training repo - changes between branches are not preserved. Each branch starts clean for consistent demos.
